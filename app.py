...
# ── Página: Gráficas ──────────────────────────────────────────
    ...
    # Selector de equipo para gráficas de jugador
    ...
    if _eq_lineup.empty:
        ...
        for _, row in _eq_lineup.iterrows():
            ...
            # ¿Entró como suplente? (entra)
            sub_in = _eq_subs[(_eq_subs["match_id"] == mid) & (_eq_subs["player_in_id"] == pid)]

            if is_starter:
                if not sub_out.empty:
                    mins = int(sub_out.iloc[0]["minute"])
                else:
                    mins = 90  # jugó todo el partido
            else:
                if not sub_in.empty:
                    mins = 90 - int(sub_in.iloc[0]["minute"])
                else:
                    mins = 0  # convocado pero no jugó
        ...
        # 1) Partidos: Titularidades y suplencias
